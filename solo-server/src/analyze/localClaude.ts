...
export async function askLocalClaude(
  ...
  // Strip ANTHROPIC_API_KEY from subprocess env → forces OAuth/keychain auth (subscription)
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      clearTimeout(timer);
      if (code !== 0) {
        reject(new Error(`Local Claude exited with code ${code}. stderr: ${stderr.slice(0, 500)}`));
        return;
      }
      resolve(stdout.trim());
    });
